...
aucpr.conf.int.expit <- function(estimate,num.pos,num.neg,conf.level=0.95) {
  ## Calculates confidence interval for an AUCPR estimate using expit. (Logit Interval)
...
confusion.matrix <- function(values,labels,threshold)
{
  ...
  return(list(tp=TP, tn=TN, fp=FP, fn=FN, precision = precision, sensitivity = sensitivity, specificity = specificity, accuracy = accuracy))
}
...
# prcurve.ap(pos.values, neg.values)
...
prcurve.ap <- function(pos.values, neg.values, conf.level=0.95, conf.int.method="binomial") {
  ...
  #stopifnot(length(pos.values) > 0)
  if(!(length(pos.values) > 0)){
    cat("length(pos.values):", length(pos.values), "\n")
    cat("length(neg.values):", length(neg.values), "\n")
    cat("print(pos.values):\n")
    print(pos.values)
  }
